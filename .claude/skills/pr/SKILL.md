---
name: pr
description: Create a feature branch, commit changes, push, and open a PR against iopsystems/llm-calc
---
...
6. **Open PR against upstream**:
   - The upstream repo is `iopsystems/llm-calc`
   - The fork remote is `origin` (determine the owner from `git remote -v`)
   - Use `gh pr create` with `--repo iopsystems/llm-calc` and `--head <fork-owner>:<branch-name>`

   ```bash
   gh pr create \
     --repo iopsystems/llm-calc \
